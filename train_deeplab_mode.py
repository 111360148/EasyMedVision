...
if analysis_names:
    ...
    batch_size = st.sidebar.number_input(
        ...
        max_value=64, 
        value=4, 
        help="A larger batch size increases training speed. If the dataset is small, a batch size of 8 or higher can be used, but it may cause memory issues."
    )
    ...
    lr = st.sidebar.number_input(
        ...
        value=1e-3, 
        format="%.6f", 
        help="A higher learning rate at the beginning can help the model converge faster, but if it's too high in later stages, it may skip the optimal solution."
    )
    ...
    if st.session_state.training_status == "Training...":
        ...
        command = f'call conda activate emv && python "{train_script_path}" --epochs {epochs} --batch {batch_size} --lr {lr}'
        ...
        with st.spinner("Training in progress..."):
            process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, 
                                    cwd=os.path.join(base_path, "DeepLabV3", "src"), text=True, bufsize=-1)
